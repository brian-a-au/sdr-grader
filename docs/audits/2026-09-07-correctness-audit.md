...
Reviewed the entire current checkout on `main`, commit `366b0834301c69b5a69a493a8bd77328710188bf` (sdr-grader 1.2.7), not merely a diff. **Nine grading defects and one additional malformed-input defect are confirmed.** The audit itself left the checkout unchanged. This PR addresses findings **5 and 6** with localized production fixes and CLI regression tests. PR #62 separately resolves the crash in finding **10** by preserving the empty fallback for malformed optional AA definitions. The remaining findings are deferred to separate iterations to keep this patch focused. All actual/expected results below describe the audited baseline, not a claim that these defects remain in the patched branch.

**Patch validation:** `tests/test_patch_grading_boundaries.py` checks category-order invariance and malformed suppression scopes through the real CLI. `tests/test_aa_definition_evidence.py` covers the retained malformed-definition fallback. The committed `observed-output.txt` is historical baseline evidence; rerunning the scripts on the patched branch is expected to differ for findings 5, 6, and 10.
